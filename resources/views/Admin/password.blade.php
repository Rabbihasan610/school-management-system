...
<head>
  ...
  <title> {{ $settings->website_name }} | Change password</title>
...
<div class="login-page-wrap">
  <div class="login-page-content bg-white">
      <div class="login-box" style="background-color: #030C3D">
          ...
          <form action="{{ route('admin.password.change') }}" method="post" class="login-form text-white">
              @csrf
              <div class="form-group">
                  <label class="text-white">New Password</label>

                  <input type="hidden" name="user_id" value="{{ $match_token->user_id}}">

                  <input type="password" placeholder="Enter New password" name="password" class="text-white form-control @error('password') is-invalid @enderror">
              ...
              @enderror
                <div class="form-group">
                    <label class="text-white">Confiram Password</label>
                    <input type="password" placeholder="Enter confiram password" name="password_confirmation" class="text-white form-control @error('password') is-invalid @enderror">
                    <i class="far fa-envelope"></i>
                </div>
                @error('password')
                <div class="alert alert-danger">{{ $message }}</div>
                @enderror
              <div class="form-group">
                  <button type="submit" class="login-btn text-white">Change Password</button>
              </div>
          </form>
